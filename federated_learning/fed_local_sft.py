...
def get_fed_local_sft_trainer(model, tokenizer, training_args, local_dataset, formatting_prompts_func, data_collator, global_dict, fed_args, script_args, local_auxiliary, global_auxiliary, packing):
    ...
    elif (fed_args.fed_alg in ['fedavg', 'clustered', 'router']) or (fed_args.fed_alg).startswith('local'):
        # Check if model has PEFT adapters (is a PeftModel)
        if isinstance(model, PeftModel):
            # For PEFT models, don't pass peft_config to avoid conflicts
            trainer = SFTTrainer(
                model=model,
                tokenizer=tokenizer,
                args=training_args,
...
class SFTTrainerFedProx(SFTTrainer):
    ...
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        for name, param in model.named_parameters():
            # Align with PEFT state dict naming
        ...
        with torch.no_grad():
            for name, param in self.model.named_parameters():
                if not param.requires_grad:
                    ...
                    name = name.replace(".default", "")
                    if name in self.global_state and name in self.correction and name in auxiliary_new_para:
...
class SCAFFOLD_Callback(TrainerCallback):
    ...
    def on_step_end(self, args, state, control, **kwargs):
        ...
        for name in list(model_para.keys()):
            if name in self.correction:
